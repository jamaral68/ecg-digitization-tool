...
def ecg_to_csv(image_name, template_name, csv_name, config_dict):
    ...
    edges = cv.Canny(foreground, 50, 150, apertureSize=3)
    ...
    print(f"Rotação de {median_angle} graus")
    
    (h, w) = image.shape[:2]
    center = (w // 2, h // 2)
    M = cv.getRotationMatrix2D(center, median_angle, 1.0)
    foreground = cv.warpAffine(foreground, M, (w, h), flags=cv.INTER_CUBIC,
                             borderMode=cv.BORDER_REPLICATE)
    image = cv.warpAffine(image, M, (w, h), flags=cv.INTER_CUBIC,
                             borderMode=cv.BORDER_REPLICATE)
    
    plt.imshow(foreground,cmap="gray")
    plt.title(f"Rotação de {median_angle} graus")
    plt.show()
...
# Extract the individual leads (lines)
    ...
    overlay = image.copy()
    for i, line in enumerate(proc_line_list):
        for curve in range(len(line['curves'])):
            x_vals = [num + proc_line_list[i]['curves'][curve]['start_y'] for num in proc_line_list[i]['curves'][curve]['xseg']] 
            y_vals = - (np.array(proc_line_list[i]['curves'][curve]['yseg']))+ proc_line_list[i]['curves'][curve]['firstpixel_abs_y'] + proc_line_list[i]['offset_line'][0]
            for k in range(len(x_vals) - 1):
                pt1 = (int(x_vals[k]), int(y_vals[k]))
                pt2 = (int(x_vals[k+1]), int(y_vals[k+1]))
                cv.line(overlay, pt1, pt2, (255, 0, 0), 5)  # Red trace overlay
   ...
   # plt.title(f'Lead {lead+1}')
    ...
    overlay.save(filename+'_dilation_'+ str(dilation) +'_overlay.png')
...
# Main program 
#filename = 'bucket/img20250311_10520856'
filename = 'dataset/images_ptbxl_gen_nodistortion/00002_hr-0'
...
num_sampling_points = time_lead/(1/sample_frequency)
location = 'left'
strategy = 'color'  # It can be filter or color
lower=(0,0,0) # black color
upper=(179,255,220) # dark gray
thres_value = 127
kSize2d = 3 
...
fig=plot_ecg(df,df.columns,csv_name, n_rows = layout[0], n_columns = layout[1], fs = 500, figure_size = (20, 12))
fig.savefig(filename+'_dilation_'+ str(dilation) +'_amplitude.png')
#plt.show()
