...
def ecg_to_csv(image_name, template_name, csv_name, config_dict):
   ...
   # plt.title(f'Lead {lead+1}')
    ...
    overlay.save('overlay/'+filename+'_dilation_'+ str(dilation) +'_overlay.png')
...
folder_path = Path('bucket/')
png_files = glob.glob(os.path.join(folder_path, '*.png'))

for file_path in png_files[5:]:
    dilation_values = [1, 5, 10]    
    for dilation in dilation_values:
        try:    
            ...
            plt.savefig('plotecg/'+filename+'_dilation_'+ str(dilation) +'_ecg.png')
            plt.show()
