...
def ecg_plot(
    ...
    dc_offset = 0
    if(show_dc_pulse):
        # add a small padding so there's an empty space after the calibration pulse
        dc_offset = sample_rate*standard_values['dc_offset_length']*step + standard_values.get('dc_offset_padding', 0)
    ...
    for i in np.arange(len(lead_index)):
        ...
        if(columns>1):
            x_offset = (i%columns)*(secs + lead_sep)
    ...
    if(full_mode!='None'):
        ...
        if(show_dc_pulse):
            # add same padding for the full lead so the waveform does not touch the calibration pulse
            dc_full_lead_offset = sample_rate*standard_values['dc_offset_length']*step + standard_values.get('dc_offset_padding', 0)
